...
class Login(cyclone.web.RequestHandler):
    ...
    credentials from the Authorization header, and uses Ubersmith as
    an authentication provider to validate that the credentials belong
    to a *client* or *contact* (not admin user).  Returns 401 on auth
    ...
    if 'Authorization' in self.request.headers:
        try:
            ...
            try:
                ...
                # call the uber.check_login method with the
                # credentials we got from basic auth
